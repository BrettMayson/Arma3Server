...
pub fn sync(mod_dir: &Path, keys_dir: &Path) -> Result<()> {
    for entry in WalkDir::new(mod_dir).into_iter().filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.is_file() && path.extension().is_some_and(|ext| ext.eq_ignore_ascii_case("bikey"))
        {
...
pub fn prepare(keys_dir: &Path, clear: bool) -> Result<()> {
    if clear && keys_dir.is_dir() {
        ...
        if a3_key.is_file() {
            std::fs::copy(&a3_key, "/tmp/keys/a3.bikey").context("copying a3.bikey to /tmp/keys")?;
        }
    ...
    if !keys_dir.is_dir() {
        ...
        if !a3_key.is_file() && Path::new("/tmp/keys/a3.bikey").is_file() {
            std::fs::copy("/tmp/keys/a3.bikey", &a3_key).context("copying a3.bikey back from /tmp/keys")?;
        }
